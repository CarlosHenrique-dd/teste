...
import { OptionsButton, SearchButton, SearchContainer } from "./styles/SearchArea"
...
const SearchArea = (props) => {
  ...
  const selecionarItensAleatorios = () => {
    const itensAleatorios = []

    while (itensAleatorios.length < 3) {
      const indiceAleatorio = Math.floor(Math.random() * movies.length)
      const itemAleatorio = movies[indiceAleatorio]
  ...
  return (
    <SearchContainer disableGutters>
      <SearchButton
        onClick={() => {
          const itensAleatorios = selecionarItensAleatorios()
          localStorage.setItem("movies", JSON.stringify(itensAleatorios))
          props.details()
        }}
      >
        Sugestão
      </SearchButton>
      <OptionsButton onClick={props.showOptions}>+</OptionsButton>
    </SearchContainer>
  )
}
